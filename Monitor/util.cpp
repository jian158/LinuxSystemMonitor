...
QPainterPath Util::generateSmoothCurve(const QList<QPointF> &points)
{
    QPainterPath path;
    int len = points.size();
    if (len < 2) {
...
void Util::calculateFirstControlPoints(double *&result, const double *rhs, int n)
{
...
void Util::calculateControlPoints(const QList<QPointF> &knots, QList<QPointF> *firstControlPoints, QList<QPointF> *secondControlPoints)
{
    int n = knots.size() - 1;
    for (int i = 0; i < n; ++i) {
...
QString Util::formatBandwidth(unsigned long v)
{
...
QString Util::formatByteCount(unsigned long v)
{
